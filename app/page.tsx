...
import {
  House, Compass, Sword, UserCircle, Plus, Sparkle, Heart, Trophy,
...
export default function Home() {
 ...
 const nav=[["Home",House],["Discover",Compass],["Battles",Sword],["Profile",UserCircle]] as const;
 ...
 return <div className="page">
   <motion.section initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} className="heroCard">
    ...
    <div className="heroCopy">
      ...
      <div className="heroActions"><button className="primary" onClick={()=>setChat(true)}><ChatCircle size={18}/> Talk to {character.name}</button><button className="secondary" onClick={()=>setBattle(discover[0])}><Sword size={18}/> Challenge</button></div>
    </div>
   ...
   <section className="memoryRow">{memories.map((m,i)=><motion.article whileHover={{y:-4}} className="memory" key={m[0]}><div className={"memoryIcon mi"+i}>{i===0?<Heart size={18}/>:i===1?<Sword size={18}/>:<Sparkle size={18}/>}</div><small>{m[2]}</small><h4>{m[0]}</h4><p>{m[1]}</p></motion.article>)}</section>
...
function PersonCard({c,onBattle}:{c:Character,onBattle:()=>void}){return <article className="person"><img src={c.avatar}/><div className="personInfo"><div><b>{c.name}</b><small>{c.handle}</small></div><span className="lv">LV {c.level}</span></div><div className="tags">{c.traits.map(t=><span key={t}>{t}</span>)}</div><button className="challenge" onClick={onBattle}>Challenge <Sword size={15}/></button></article>}
